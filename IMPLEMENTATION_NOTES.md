...
The `OUTER_SAMPLE` wrapper owns the two-pass lifetime. It clones the original packed noise, latent, sigma schedule, and optional mask before the first call. The first pass runs ordinary Spectrum without the external callback. Actual-step finalization retains canonical model-dtype anchors on the configured history device before causal history eviction. The causal forecaster and archive share immutable owned tensors while an anchor remains in both, so an eleven-anchor replay with `max_history=8` owns eleven feature tensors rather than nineteen. Explicit `vram` storage keeps the full offline archive on the producing device; system-RAM storage keeps it on CPU.
...
The smoother withholds each interior anchor and evaluates a deterministic sample of at most 16,384 elements per conditional branch for the packed audio and video segments independently. It compares leave-one-out spectral RMS error with adjacent-anchor interpolation RMS error. The worse nearby audio/video ratio attenuates the configured spectral share for a missing step by `1 / max(1, score)`. This keeps `blend_weight` as an upper bound, protects the smaller audio segment from being hidden by video-dominated aggregate error, and retains one packed prediction buffer. Stored actual steps use one-hot weights and therefore reproduce their archived feature exactly. Telemetry reports validation samples, anchors, streams, per-modality maxima, effective blend range/mean, attenuated predictions, exact-anchor versus smoothed replay steps, and the smoother's history/chunk state.

Replay opens a fresh runtime run from the cloned inputs. Because `CFGGuider.inner_sample` replaces `guider.conds` with processed conditions, the wrapper also restores a structural copy of the pre-first-pass condition containers; tensors and model/control payloads remain shared. Every step has runtime mode `replay`, so `DIFFUSION_MODEL` obtains an archived or smoothed target and never enters `_execute_actual`. Replay validates coordinates, topology, labels, row coverage, and output shape. An `OfflineReplayAbort` unwinds the second ComfyUI sampling call transactionally and lets the outer wrapper return the already valid first-pass result. OOM and cancellation remain fatal and preserve their original traceback. The outer `finally` releases input clones, the selected-device archive, and smoother references.
