...
| `blend_weight` | `0.50` | Video spectral share. The remainder is a two-point local linear prediction. |
...
MiniMax H3 packs generated audio and video into one transformer sequence while conditioning their rows on different shifted timestep schedules. Earlier revisions applied one `blend_weight` to the complete packed target. In two matched runs with `blend_weight=0.5`, both ordinary Spectrum and offline replay produced an audio defect on the tested seed; the corresponding two `blend_weight=0` runs did not. The offline run also measured a worst held-out spectral/local error ratio of `3.095` for audio and still retained `0.162-0.309` spectral share, showing that proportional attenuation did not establish perceptual safety.
...
A subsequent full-H3 comparison showed that direct row separation was necessary but insufficient: `video=0.5, audio=0` still produced slight speech tripping, `video=1, audio=0` was nearly clean, and `video=0, audio=0` was clean with somewhat worse image quality. The non-monotonic result rules out a simple model in which speech damage scales with the video spectral share. The remaining causal path is the first-pass joint transformer: a video forecast changes the solver trajectory, and the following actual H3 call jointly attends over audio and video, so even directly local-only audio rows can produce altered later audio anchors. Offline replay therefore captures both modalities with causal weights fixed at zero. The configured per-modality weights are applied only during transformer-free replay, after the audio-safe anchor trajectory is complete. Ordinary one-pass Spectrum keeps the configured weights unchanged.
